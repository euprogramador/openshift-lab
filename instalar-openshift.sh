...
ansible nodes -i inventories/openshift-in-ha-mode.erb -c local -m shell -a "if [[ {{ groups['masters'] | length }} -eq 1 ]]; then exit 0; else exit 1; fi" 2>&1 > /dev/null
if [ $? -eq 0 ]; then
    print 100
    print 100 "      [SKIP] - há apenas um servidor master."
    print 100 "               não será configurado HA (High Avaliability) para servidores masters"
else
    ANSIBLE_HOST_KEY_CHECKING=False sshpass -p root ansible-playbook -i inventories/$INVENTORY.erb playbooks/openshift-ha-playbook/site-ha-masters.yml -u root --ask-pass 2>&1 > logs/install-ha-masters.log
    if [ $? -eq 0 ]; then
...
ansible nodes -i inventories/openshift-in-ha-mode.erb -c local -m shell -a "if [[ {{ groups['lb-app-nodes'] | length }} -eq 1 ]]; then exit 0; else exit 1; fi" 2>&1 > /dev/null
if [ $? -eq 0 ]; then
    print 100
    print 100 "      [SKIP] - há apenas um servidor infra."
    print 100 "               não será configurado HA (High Avaliability) para servidores infra"
else
    ANSIBLE_HOST_KEY_CHECKING=False sshpass -p root ansible-playbook -i inventories/$INVENTORY.erb playbooks/openshift-ha-playbook/site-ha-infra.yml -u root --ask-pass 2>&1 >logs/install-ha-infra.log
    if [ $? -eq 0 ]; then
...
ANSIBLE_HOST_KEY_CHECKING=False sshpass -p root ansible-playbook -i inventories/masters-ha-etcd-external-calico.erb tmp/openshift-ansible/playbooks/prerequisites.yml -u root --ask-pass

ANSIBLE_HOST_KEY_CHECKING=False sshpass -p root ansible-playbook -i inventories/masters-ha-etcd-external-calico.erb tmp/openshift-ansible/playbooks/deploy_cluster.yml -u root --ask-pass
